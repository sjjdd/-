...
double globalsoil=5000;//全局变量存储浇水值
...
void loop() {
  ...
  client.publish("SoilHum190418201",msg);
  delay(1000);
 }
...
void reconnect(){
  ...
  while (!client.connected()) {
    ...
    if (client.connect(mqtt_username, mqtt_username, mqtt_password)) {
      ...
      //去实时地订阅土壤湿度阈值
      client.subscribe("setwaterparam190418201");
      client.subscribe("SoilHum190418201");
      client.subscribe("soil190418201");
  } else {
 ...
 void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,"setwaterparam190418201")==0)
  {
    globalsoil=atof(inchars.c_str());
  }

   if(strcmp(topic,"soil190418201")==0){
     if(inchars=="1")
    {
   ...
   if(strcmp(topic,"SoilHum190418201")==0&&jbj==0){
